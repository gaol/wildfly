...
| session-tracking-modes | The global session tracking modes for all deployments
to the servlet container. This can also be set via system propety
 `undertow.session.tracking-modes` + `.CONTAINER-NAME`, `undertow.session.tracking-modes`
is used if the container name is omitted.
